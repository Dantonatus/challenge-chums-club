-- Create function to get popular challenges by duration
...
BEGIN
    ...
    active_days AS (
        SELECT id, duration_days AS active_days FROM duration
    ),
    fail_rate AS (
        SELECT
            a.id,
    ...
    FROM duration d
    LEFT JOIN participants p USING (id)
    LEFT JOIN fails f USING (id)
    LEFT JOIN fail_rate fr USING (id)
    ORDER BY 
