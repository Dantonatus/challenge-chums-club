...
-- Helper: check group membership/ownership
...
CREATE POLICY "Group owner can manage group" ON public.groups
...
CREATE POLICY "Owner can add members" ON public.group_members
FOR INSERT TO authenticated
WITH CHECK (public.is_group_owner(group_id));

CREATE POLICY "Owner or self can remove membership" ON public.group_members
...
CREATE POLICY "Owner or creator can update challenges" ON public.challenges
...
CREATE POLICY "Owner or creator can delete challenges" ON public.challenges
FOR DELETE TO authenticated
USING (public.is_group_owner(group_id) OR created_by = auth.uid());
...
CREATE POLICY "Members can view challenge participants" ON public.challenge_participants
FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM public.challenges c WHERE c.id = challenge_id AND public.is_group_member(c.group_id))
);

CREATE POLICY "Owner or creator can modify challenge participants" ON public.challenge_participants
FOR ALL TO authenticated
USING (
...
CREATE POLICY "Members can view logs in group" ON public.logs
FOR SELECT TO authenticated
USING (
  EXISTS (
...
CREATE POLICY "Participants can write their own logs" ON public.logs
FOR INSERT TO authenticated
WITH CHECK (
...
CREATE POLICY "Participants can update/delete their own logs" ON public.logs
FOR UPDATE TO authenticated
USING (user_id = auth.uid())
WITH CHECK (user_id = auth.uid());

CREATE POLICY "Participants can delete their own logs" ON public.logs
FOR DELETE TO authenticated
USING (user_id = auth.uid());
...
CREATE POLICY "Members can view payments in group" ON public.payments
FOR SELECT TO authenticated
USING (public.is_group_member(group_id));

CREATE POLICY "Owner can record owed/adjustment; users can mark their own paid" ON public.payments
FOR INSERT TO authenticated
WITH CHECK (
...
CREATE POLICY "Group members can view ideas" ON public.ideas
FOR SELECT TO authenticated
USING (public.is_group_member(group_id));
...
CREATE POLICY "Author or owner can update ideas" ON public.ideas
FOR UPDATE TO authenticated
...
CREATE POLICY "Author or owner can delete ideas" ON public.ideas
FOR DELETE TO authenticated
USING (created_by = auth.uid() OR public.is_group_owner(group_id));
...
CREATE POLICY "Members can view comments" ON public.idea_comments
FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM public.ideas i WHERE i.id = idea_id AND public.is_group_member(i.group_id))
);

CREATE POLICY "Members can comment" ON public.idea_comments
FOR INSERT TO authenticated
WITH CHECK (
  user_id = auth.uid() AND EXISTS (SELECT 1 FROM public.ideas i WHERE i.id = idea_id AND public.is_group_member(i.group_id))
);

CREATE POLICY "Authors can delete their comments" ON public.idea_comments
FOR DELETE TO authenticated
USING (user_id = auth.uid());

-- Journal entries (private to user by default)
...
CREATE POLICY "Users can view their own journal" ON public.journal_entries
FOR SELECT TO authenticated
USING (user_id = auth.uid());

CREATE POLICY "Users can write their own journal" ON public.journal_entries
FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());
...
-- Scheduled mini-challenges (conversation tips)
...
CREATE POLICY "Members can view scheduled tips" ON public.scheduled_tips
FOR SELECT TO authenticated
USING (public.is_group_member(group_id));

CREATE POLICY "Members can schedule tips for their group" ON public.scheduled_tips
FOR INSERT TO authenticated
WITH CHECK (public.is_group_member(group_id) AND user_id = auth.uid());

CREATE POLICY "Owner or creator can delete scheduled tips" ON public.scheduled_tips
FOR DELETE TO authenticated
USING (public.is_group_owner(group_id) OR user_id = auth.uid());
