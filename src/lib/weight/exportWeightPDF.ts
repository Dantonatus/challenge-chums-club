...
export async function exportWeightPDF(
  ...
  doc.text('Gewichtsbericht', MARGIN, 13);

  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(format(new Date(), 'dd.MM.yyyy'), PAGE_W - MARGIN, 13, { align: 'right' });
  ...
  doc.save(`gewicht-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
}
