...
export function CompareBar({ participants, onSelectionChange, lang }: CompareBarProps) {
  ...
  const availableForA = participants.filter(p => p !== selectedB);
  const availableForB = participants.filter(p => p !== selectedA);
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50/50 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/20 rounded-xl border border-blue-200/50 dark:border-blue-800/30">
              ...
              <CommandList>
                {availableForA.map((participant) => (
                  <CommandItem
                    key={participant}
                    value={participant}
                    onSelect={() => handleSelectionA(participant)}
                    className="cursor-pointer"
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        selectedA === participant ? "opacity-100" : "opacity-0"
                      )}
                    />
                    {participant}
                  </CommandItem>
              ...
              <CommandList>
                {availableForB.map((participant) => (
                  <CommandItem
                    key={participant}
                    value={participant}
                    onSelect={() => handleSelectionB(participant)}
                    className="cursor-pointer"
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        selectedB === participant ? "opacity-100" : "opacity-0"
                      )}
                    />
                    {participant}
                  </CommandItem>
      ...
      {/* Status Badge */}
      {selectedA && selectedB && (
        <Badge variant="secondary" className="bg-green-100 text-green-800 border-green-200 dark:bg-green-950 dark:text-green-200 dark:border-green-800">
